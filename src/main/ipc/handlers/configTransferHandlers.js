...
const configTransferService = require("../../settings/configTransferService");
...
class ConfigTransferHandlers {
  ...
  _notifyConfigImported(appliedSections = []) {
    if (
      appliedSections.includes("connections") ||
      appliedSections.includes("aiSettings")
    ) {
    ...
    if (appliedSections.length > 0) {
      ...
      for (const win of windows) {
        if (win && !win.isDestroyed() && win.webContents) {
          try {
            win.webContents.send(IPC_EVENT_CHANNELS.CONFIG_TRANSFER_IMPORTED, {
  ...
  async loadSyncSettings() {
    return {
      success: true,
      settings: configTransferService.loadSyncSettings(),
    };
  }
