...
const recentTransfers = ref<
  { id: string; filename: string; type: TransferType; status: string }[]
>([]);
// 在没有显示传输窗口时显示的最新传输提示
...
const contextMenuTarget = ref<"file" | "directory" | "background">(
  "background",
);
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (result.success && result.files) {
          ...
          console.error(
            `目录加载失败 (尝试 ${retryCount + 1}/${maxRetries}):`,
            result.error,
          );
        ...
        console.error(
          `加载目录时发生错误 (尝试 ${retryCount + 1}/${maxRetries}):`,
          err,
        );
      ...
      try {
        ...
        if (!uploadResult.success) {
          if (uploadResult.failedFiles && uploadResult.failedFiles.length > 0) {
            ...
            console.error(
              "上传失败的文件:",
              uploadResult.failedFiles.map((f) => path.basename(f)).join(", "),
            );
        ...
        error.value =
          err instanceof Error ? err.message : "批量上传文件时发生错误";
      }
    }
  } catch (err) {
...
const handleDragOver = (
  e: DragEvent,
  targetType: "file" | "directory" | "background" = "background",
  targetName?: string,
) => {
  e.preventDefault();
  e.stopPropagation();
  ...
  try {
    ...
    if (files.length > 1) {
      ...
      const filePathPromises = files.map(async (file) => {
        ...
        if (!filePath) {
          console.log(
            `文件 ${file.name} 没有路径信息，将使用FileReader读取内容`,
          );
      ...
      try {
        ...
        // 清理临时文件
        const tempFilePaths = filePaths.filter(
          (path) => !files.some((file) => (file as any).path === path),
        );
      ...
      for (const file of files) {
        try {
          ...
          if (!filePath) {
            console.log(
              `文件 ${fileName} 没有路径信息，将使用FileReader读取内容`,
            );
    ...
    if (files.length > 1) {
      ...
      for (const file of files) {
        try {
          ...
          console.error(
            `上传文件 ${file.name} 到文件夹 ${folderName} 时发生错误:`,
            fileError,
          );
          error.value =
            fileError instanceof Error
    ...
    for (const fileName of itemsToDelete) {
      ...
      try {
        ...
        if (!result.success) {
          throw new Error(
            result.error ||
              `删除${fileType === "file" ? "文件" : "文件夹"} ${fileName} 失败`,
          );
        }
  ...
  setTimeout(() => {
    ...
    nextTick(() => {
      ...
      if (menuElement) {
        ...
        // 如果位置有调整，应用新位置
        if (
          adjustedX !== menuPosition.value.x ||
          adjustedY !== menuPosition.value.y
        ) {
...
const adjustTransferWindowPosition = () => {
  ...
  const transferWindow = document.querySelector(
...
const initFileTransferHandlers = () => {
  ...
  const unsubTransferProgress = window.api.onTransferProgress((data) => {
    // 查找对应的传输项
    const transferItem = transferProgress.value.find(
...
const scheduleItemRemoval = (item: TransferItem, delay = 5000) => {
  ...
  console.log(
    `安排移除传输项：${item.filename}，状态：${item.status}，延迟：${delay}ms`,
  );
  ...
  const timerId = window.setTimeout(() => {
    ...
    nextTick(() => {
      ...
      console.log(
        `还有活跃传输: ${hasActiveTransfers}, 总数: ${transferProgress.value.length}`,
      );
...
const checkTransferComplete = () => {
  ...
  if (allCompleted && transferProgress.value.length > 0) {
    ...
    const errorCount = transferProgress.value.filter(
  ...
  if (transferItem) {
    ...
    nextTick(() => {
      ...
      transferProgress.value = transferProgress.value.filter(
        (item) =>
          item.id !== transferId ||
          (item.id === transferId && !item.pendingRemoval),
      );
      ...
      if (stuckItems.length > 0) {
        ...
        stuckItems.forEach((item) => {
          ...
          if (item.status === "transferring" && item.progress === 100) {
            console.log(
              `定期检查：修复卡在transferring的项目: ${item.filename}`,
            );
            item.status = "verifying";
    ...
    if (item.type === "upload") {
      ...
      if (result.success && result.files) {
        ...
        if (uploadedFile && uploadedFile.size === item.size) {
          console.log(
            `上传验证成功: ${item.filename}, 大小匹配: ${uploadedFile.size}`,
          );
          return true;
        } else {
          console.error(
            `上传验证失败: ${item.filename}, 文件不存在或大小不匹配`,
          );
          return false;
        }
      }
...
// 处理传输验证结果
const handleTransferVerificationResult = (
  item: TransferItem,
  success: boolean,
) => {
  if (success) {
...
const handleColumnDragMove = (e: MouseEvent) => {
  ...
  // 添加拖动指示器
  const indicator =
    document.getElementById("column-resize-indicator") ||
...
const loadColumnWidths = () => {
  try {
    ...
    if (savedWidths) {
      ...
      // 确保所有必要的列都存在
      const requiredColumns: (keyof ColumnWidths)[] = [
        "checkbox",
        "name",
        "size",
        "time",
      ...
      const isValid = requiredColumns.every(
        (col) => typeof parsedWidths[col] === "number",
      );
...
const adjustColumnsToFitContainer = () => {
  ...
  if (containerWidth > 0) {
    ...
    // 计算所有非name列所需的总宽度
    const otherColumnsMinWidth =
      minWidths.checkbox +
      minWidths.size +
      minWidths.time +
      minWidths.permissions +
      minWidths.owner;

    // 给name列分配剩余的所有空间
    let nameWidth = Math.max(
      minWidths.name,
      containerWidth - otherColumnsMinWidth,
    );
    ...
    if (containerWidth < otherColumnsMinWidth + minWidths.name) {
      ...
      // 其他列按比例分配剩余空间
      newWidths.size = Math.max(
        30,
        Math.floor(remainingWidth * (minWidths.size / otherColumnsMinTotal)),
      );
      newWidths.time = Math.max(
        30,
        Math.floor(remainingWidth * (minWidths.time / otherColumnsMinTotal)),
      );
      newWidths.permissions = Math.max(
        30,
        Math.floor(
          remainingWidth * (minWidths.permissions / otherColumnsMinTotal),
        ),
      );
      newWidths.owner = Math.max(
        30,
        Math.floor(remainingWidth * (minWidths.owner / otherColumnsMinTotal)),
      );
    } else {
      // 空间足够，保持其他列的最小宽度
...
// 处理文件拖动开始
const handleFileDragStart = async (
  e: DragEvent,
  fileName: string,
  fileType: "file" | "directory",
) => {
  ...
  try {
    ...
    // 首先需要下载文件到本地临时目录
    const tempFilePath = await window.electron.ipcRenderer.invoke(
    ...
    if (files.length > 1) {
      ...
      for (const file of files) {
        try {
          ...
          console.log(
            `准备上传文件: ${fileName} 从 ${filePath} 到 ${targetPath}`,
          );
  ...
  <div class="file-manager" :class="{ 'dark-theme': isDarkTheme }">
    ...
    >
      ...
      <button
        ...
        @click="latestTransferNotification.visible = false"
      >
        ×
      </button>
    </div>
    ...
    >
        ...
        >
          ...
          >
            <input
              type="checkbox"
              :checked="
                selectedFiles.size === fileList.length && fileList.length > 0
              "
              :indeterminate="
                selectedFiles.size > 0 && selectedFiles.size < fileList.length
              "
              @change="
                (e) => {
          ...
          >
            ...
            <div
              ...
              @mousedown.stop="(e) => handleColumnDragStart(e, 'name')"
            ></div>
          </div>
          <div
          ...
          >
            ...
            <div
              class="column-resize-handle"
              @mousedown.stop="(e) => handleColumnDragStart(e, 'size')"
            ></div>
          </div>
          <div
          ...
          >
            ...
            <div
              class="column-resize-handle"
              @mousedown.stop="(e) => handleColumnDragStart(e, 'time')"
            ></div>
          </div>
          <div
      ...
      <div v-else class="file-list">
        <div
          ...
          @dragover.stop="
            file.type === 'directory'
              ? handleFolderDragOver($event, file.name)
          ...
          @drop.stop="
            file.type === 'directory'
              ? handleFolderDrop($event, file.name)
      ...
      <div v-if="deleteProgress.isDeleting" class="delete-progress">
        ...
        <div class="progress-bar-container">
          <div
            class="progress-bar"
            :style="{
              width: `${(deleteProgress.completed / deleteProgress.total) * 100}%`,
            }"
          ></div>
        </div>
      </div>
      ...
      >
        ...
        <template v-else>
          ...
          <template v-if="selectedFiles.size > 0">
            <div
              class="menu-item delete-menu-item"
    ...
    >
      <div class="transfer-header" @mousedown="startDragTransferWindow">
        ...
        <div class="transfer-header-right">
          ...
          >
          <button class="transfer-close-btn" @click="closeTransferProgress">
            ×
          </button>
        </div>
      </div>
      ...
      <div v-if="!isTransferWindowCollapsed" class="transfer-items">
        ...
        >
          <div class="transfer-item-header">
            <span class="transfer-filename" :title="item.filename">{{
              item.filename
            }}</span>
    ...
    <teleport to="body">
      ...
      >
        <div
          ...
          :class="{ 'dark-dialog': props.isDarkTheme }"
          @click.stop
        >
          <div class="dialog-header">
            <h3>
              {{ fileInfo?.type === "directory" ? "文件夹信息" : "文件信息" }}
            </h3>
          ...
          <div v-if="fileInfo" class="dialog-content">
            ...
            <div class="info-row">
              ...
              <span class="info-value">{{
                formatDateTime(fileInfo.modifyTime)
              }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">访问时间:</span>
              <span class="info-value">{{
                formatDateTime(fileInfo.accessTime)
              }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">权限:</span>
              <span class="info-value">{{
                formatPermissions(fileInfo.rights)
              }}</span>
            </div>
            <div class="info-row">
          ...
          <div class="dialog-footer">
            <button class="close-button-text" @click="closeInfoDialog">
              关闭
            </button>
          </div>
        </div>
