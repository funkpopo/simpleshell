<template>
  <div class="connection-dialog-wrapper" @click.self="closeModal">
    <div class="connection-dialog" :class="{'organization-mode': isOrganization}">
      <div class="dialog-header">
        <h2>{{ isEditMode ? t(isOrganization ? 'connection.editOrganization' : 'connection.editConnection') : t(isOrganization ? 'connection.newOrganization' : 'connection.newConnection') }}</h2>
      ...
      <div class="dialog-content">
        ...
        <div class="form-group">
          <label for="name">{{ t('connection.name') }} <span class="required">*</span></label>
          <input 
            type="text" 
            id="name" 
            ...
            @input="validateForm"
          />
          <div class="error-message" v-if="validation.name">{{ validation.name }}</div>
        </div>
        
        <!-- 连接信息 (仅在非组织模式下显示) -->
        <template v-if="!isOrganization">
          <div class="form-group">
            <label for="host">{{ t('connection.host') }} <span class="required">*</span></label>
            <input 
              type="text" 
              id="host" 
              ...
              @input="validateForm"
            />
            <div class="error-message" v-if="validation.host">{{ validation.host }}</div>
          </div>
          
          <div class="form-group">
            <label for="port">{{ t('connection.port') }} <span class="required">*</span></label>
            <input 
              type="number" 
              id="port" 
              ...
              @input="validateForm"
            />
            <div class="error-message" v-if="validation.port">{{ validation.port }}</div>
            <div class="hint-text" v-else>{{ t('connection.defaultPort') }}</div>
          </div>
          
          <div class="form-group">
            <label for="username">{{ t('connection.username') }} <span class="required">*</span></label>
            <input 
              type="text" 
              id="username" 
              ...
              @input="validateForm"
            />
            <div class="error-message" v-if="validation.username">{{ validation.username }}</div>
          </div>
          ...
          <div class="form-group">
            ...
            <input 
              ...
              @input="validateForm"
            />
            <div class="error-message" v-if="validation.password">{{ validation.password }}</div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="file-input-wrapper">
              ...
              <button class="clear-file-btn" @click="clearPrivateKey" v-if="formData.privateKey">
                {{ t('connection.clear') }}
              </button>
            </div>
            <div class="error-message" v-if="validation.privateKey">{{ validation.privateKey }}</div>
            <div class="error-message" v-if="validation.authMethod">{{ validation.authMethod }}</div>
          </div>
