...
const searchResults = ref<{ count: number; current: number }>({
  count: 0,
  current: 0,
});
...
const addTab = (
  ...
  const id = generateId();
  console.log(
    `创建新标签页 ${name}, ID: ${id}, 是本地终端: ${isLocalTerminal}`,
  );
...
const switchToTab = (id: string) => {
  ...
  if (tab) {
    ...
    // 通知父组件当前活动连接ID变化
    const connectionId = tab.isLocalTerminal
      ? null
      : tab.connection?.id || null;
...
const closeTab = (id: string) => {
  ...
  try {
    ...
    // 检查是否关闭的是当前活动的SSH连接
    const wasActiveSSHConnection =
      tab.isActive && !tab.isLocalTerminal && tab.connection;
    ...
    if (
      wasActiveSSHConnection &&
      nextConnectionId === null &&
      tabs.value.length > 0
    ) {
      console.log(
        "关闭了活动的SSH连接，新的活动标签页可能是本地终端，显式通知连接ID变更为null",
      );
      emit("active-connection-change", null);
    }
...
const initializeTerminal = async (tab: TerminalTab) => {
  ...
  nextTick(() => {
    ...
    // 检查是否已有元素，有则删除
    const existingElement = terminalContainer.value.querySelector(
      `[data-tab-id="${tab.id}"]`,
    );
    if (existingElement) {
    ...
    terminalElement.addEventListener("contextmenu", (e) =>
      handleContextMenu(e, tab.terminal!),
    );
    terminalElement.addEventListener("mousedown", (e) =>
      handleMiddleClick(e, tab.terminal!),
    );
    ...
    nextTick(() => {
      ...
      const findXtermTextarea = () => {
        const xtermTextarea = terminalElement.querySelector(
          ".xterm-helper-textarea",
        ) as HTMLTextAreaElement;
        if (xtermTextarea) {
          xtermTextarea.addEventListener("keydown", handleKeyDown);
          console.log(
            `为标签页 ${tab.id} 的 xterm textarea 添加了键盘事件监听`,
          );
          return true;
        }
        return false;
      ...
      if (!findXtermTextarea()) {
        console.log(
          `首次查找标签页 ${tab.id} 的 xterm textarea 失败，将在 100ms 后重试`,
        );
        // 如果没找到，延迟 100ms 后再次尝试
        setTimeout(() => {
          if (!findXtermTextarea()) {
            console.warn(
              `无法找到标签页 ${tab.id} 的 xterm textarea 元素，将使用全局事件监听作为备选方案`,
            );
          }
        }, 100);
      }
  ...
  nextTick(() => {
    try {
      ...
      if (
        ...
        const xtermScreen = targetTab.terminalElement.querySelector(
          ".xterm-screen",
        ) as HTMLElement;
        if (xtermScreen) {
...
const handleTabResize = (tab: TerminalTab) => {
  ...
  if (tab.status === "connected") {
    if (tab.isLocalTerminal && tab.terminalId) {
      console.log(
        `向本地终端[${tab.terminalId}]发送调整大小命令: ${cols}x${rows}`,
      );
      try {
        api.resizeTerminal({
          id: tab.terminalId,
      ...
      console.log(
        `向SSH终端[${tab.connectionId}/${tab.shellId}]发送调整大小命令: ${cols}x${rows}`,
      );
      try {
        ...
        console.error(
          `调整SSH终端[${tab.connectionId}/${tab.shellId}]大小失败:`,
          error,
        );
      }
    }
    ...
    console.log(
      `终端未连接，不发送调整大小命令，标签ID: ${tab.id}, 状态: ${tab.status}`,
    );
  }
};
...
const connectToSSH = async (tab: TerminalTab) => {
  ...
  try {
    ...
    // 显示连接中信息
    tab.terminal.write(
      `正在连接到 ${tab.connection.name} (${tab.connection.host}:${tab.connection.port})...\r\n`,
    );
    ...
    try {
      ...
      const errorMessage =
        connectError instanceof Error ? connectError.message : "未知错误";
    ...
    try {
      ...
      const errorMessage =
        shellError instanceof Error ? shellError.message : "未知错误";
      throw new Error(`创建Shell失败: ${errorMessage}`);
    }
    ...
    tab.status = "error";
    tab.errorMessage = error instanceof Error ? error.message : "连接失败";
...
const connectToLocalTerminal = async (tab: TerminalTab) => {
  ...
  try {
    ...
    tab.status = "connected";
    console.log(
      `本地终端连接成功，标签ID: ${tab.id}, 终端ID: ${tab.terminalId}`,
    );
    ...
    const dataUnsubscribe = api.onTerminalData((event) => {
      ...
      if (event.id === tab.terminalId && tab.terminal) {
        try {
          ...
          console.error(
            `写入终端数据错误 (标签ID: ${tab.id}, 终端ID: ${tab.terminalId}):`,
            err,
          );
        }
      } else if (
        event.id !== tab.terminalId &&
        event.data &&
        event.data.length > 0 &&
        process.env.NODE_ENV === "development"
      ) {
        ...
        console.log(
          `标签页[${tab.id}]收到了不属于自己的终端[${event.id}]数据，但未处理`,
        );
      }
    });
    ...
    tab.status = "error";
    tab.errorMessage = error instanceof Error ? error.message : "启动终端失败";
...
const disconnectTerminal = (tab: TerminalTab) => {
  console.log(
    `执行disconnectTerminal函数，标签ID: ${tab.id}, 终端ID: ${tab.terminalId}`,
  );

  // 断开SSH连接
  if (tab.connectionId && tab.shellId) {
    console.log(
      "断开SSH连接, connectionId:",
      tab.connectionId,
      "shellId:",
      tab.shellId,
    );
...
const addLocalTerminal = () => {
  ...
  if (now - lastTerminalCreationTime.value < TERMINAL_CREATION_DEBOUNCE_MS) {
    console.log(
      `终端创建请求过于频繁，已忽略 (${now - lastTerminalCreationTime.value}ms)`,
    );
    return "";
  }
  ...
  // 生成唯一的标签名，避免重复
  const terminalCount =
    tabs.value.filter((tab) => tab.isLocalTerminal).length + 1;
  const tabName =
    terminalCount > 1 ? `本地终端 (${terminalCount})` : "本地终端";
...
const addSshConnection = (connection: Connection) => {
  try {
    ...
    const newTabId = addTab(
      `${cleanConnection.name} (${cleanConnection.host})`,
      false,
      cleanConnection,
    );
    ...
    errorMessage.value = `无法建立SSH连接: ${errorMsg}`;
    return null;
  }
};
...
const handleGlobalKeyDown = (event: KeyboardEvent) => {
  if (event.ctrlKey && event.key === "f" && activeTab.value) {
    ...
    if (
      target.closest(".terminal-instance") ||
...
onMounted(() => {
  ...
  if (props.isLocalMode && tabs.value.length === 0) {
    ...
    const isCreatingFromParent =
      window.localStorage.getItem("terminal_creating");
...
const handleDrop = (targetTabId: string, event: DragEvent) => {
  ...
  const draggedIndex = tabs.value.findIndex(
  ...
  try {
    ...
    const options = {
      ...
      decorations: {
        ...
        activeMatchColorOverviewRuler: props.isDarkTheme
          ? "#ffffff"
          : "#000000",
      },
    };
    ...
    // 执行搜索
    const result = activeTab.value.searchAddon.findNext(
      searchText.value,
      options,
    );
    ...
    if (result) {
      // 计算匹配数量
      const count = estimateSearchMatches(
        activeTab.value.terminal,
        searchText.value,
        options,
      );
  ...
  try {
    ...
    if (result && searchResults.value.count > 0) {
      searchResults.value.current =
        (searchResults.value.current % searchResults.value.count) + 1;

      // 确保当前匹配项可见
      activeTab.value.terminal.scrollToLine(
        activeTab.value.terminal.buffer.active.cursorY,
      );
    }
  } catch (error) {
...
const findPrevious = () => {
  ...
  try {
    ...
    const result = activeTab.value.searchAddon.findPrevious(
      searchText.value,
      options,
    );
...
const estimateSearchMatches = (
  ...
  try {
    ...
    try {
      ...
      return matchCount > 0
        ? matchCount
        : Math.max(1, Math.min(10, Math.floor(lineCount / 10)));
    } catch (error) {
  ...
  tabs.value.forEach((tab) => {
    if (tab.terminal) {
      tab.terminal.options.fontFamily =
        terminalSettings.value.terminalFontFamily;
...
onMounted(async () => {
  ...
  api.onSettingsChanged((newSettings: GlobalSettings) => {
    console.log("检测到设置变更:", newSettings);
    if (newSettings) {
      // 检查终端字体或字号是否变化
      const fontChanged =
        terminalSettings.value.terminalFontFamily !==
        newSettings.terminalFontFamily;
      const sizeChanged =
        terminalSettings.value.terminalFontSize !==
        newSettings.terminalFontSize;
      ...
      <div v-if="showSearchBar" class="search-container">
        <div class="search-box">
          ...
          <div class="search-actions">
            ...
            <button class="search-close" title="关闭搜索" @click="closeSearch">
              <span>×</span>
            </button>
          </div>
        </div>
