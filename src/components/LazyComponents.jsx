...
// 文件管理器懒加载组件
const FileManager = lazy(() => import("./FileManager.jsx").catch((error) => {
  // 返回一个默认的错误组件
  return {
    default: () => (
...
// 资源监控懒加载组件
const ResourceMonitor = lazy(() => import("./ResourceMonitor.jsx").catch((error) => {
  return {
    default: () => (
      <div style={{ padding: "20px", textAlign: "center" }}>
        资源监控组件加载失败，请刷新页面重试
...
// AI助手懒加载组件
const AIAssistant = lazy(() => import("./AIAssistant.jsx").catch((error) => {
  return {
    default: () => (
      <div style={{ padding: "20px", textAlign: "center" }}>
        AI助手组件加载失败，请刷新页面重试
...
// IP地址查询组件的懒加载实现
const IPAddressQuery = lazy(() => import("./IPAddressQuery.jsx").catch((error) => {
  return {
    default: () => (
      <div style={{ padding: "20px", textAlign: "center" }}>
...
// 导出懒加载组件以供直接使用（如果需要）
export { FileManager, ResourceMonitor, AIAssistant, IPAddressQuery, preloadComponents };
