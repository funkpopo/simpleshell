    ...
    (
      ...
      const [splitState, setSplitState] = useState({
        ...
        panes: [
          {
            id: "1",
            position: PANE_POSITIONS.TOP_LEFT,
            tabId: null,
            tabIndex: null,
          },
          {
            id: "2",
            position: PANE_POSITIONS.TOP_RIGHT,
            tabId: null,
            tabIndex: null,
          },
          {
            id: "3",
            position: PANE_POSITIONS.BOTTOM_LEFT,
            tabId: null,
            tabIndex: null,
          },
          {
            id: "4",
            position: PANE_POSITIONS.BOTTOM_RIGHT,
            tabId: null,
            tabIndex: null,
          },
        ],
      ...
      // 启用分屏模式
      const enableSplitLayout = useCallback(
        (
          layout = LAYOUT_TYPES.DUAL_HORIZONTAL,
          primaryTabIndex = currentTab,
          ...
          if (
            ...
            // 设置副分屏
            const secondaryPaneIndex =
              layout === LAYOUT_TYPES.DUAL_HORIZONTAL
                ? 1
                : layout === LAYOUT_TYPES.DUAL_VERTICAL
                  ? 2
                  : 1;
      ...
      // 处理标签页拖拽到分屏区域
      const handleTabDropToPane = useCallback(
        (paneId, tabIndex) => {
      ...
      const getActivePanes = useCallback(() => {
        ...
        switch (splitState.layout) {
          case LAYOUT_TYPES.DUAL_HORIZONTAL:
            return splitState.panes.filter(
              (pane) =>
      ...
      return (
        <Box ref={layoutRef} sx={getLayoutStyles()}>
          {activePanes.map((pane) => (
            <SplitPane
              ...
              terminalInstance={
                pane.tabId ? terminalInstances[pane.tabId] : null
              }
