...
const MessageContent = styled(Paper)(({ theme, isuser }) => ({
  ...
  maxWidth: '85%',
  minWidth: '40px',
  backgroundColor: isuser === 'true' ? theme.palette.primary.main : 
...
function AIAssistant({ open, onClose }) {
  ...
  const [messages, setMessages] = React.useState([
    { id: 'welcome-1', text: '你好！我是AI助手，有什么可以帮助您的吗？', isUser: false }
  ]);
  ...
  const sendRequestViaElectron = async (prompt, settings = apiSettings.current) => {
    try {
      const messages = [
        { role: "system", content: "你是一个有帮助的助手。" },
        { role: "user", content: prompt }
      ...
      if (error.message === 'Failed to fetch' || error.message.includes('网络连接失败')) {
        throw new Error('网络连接失败: 无法连接到API服务器。请检查您的网络连接、API URL是否正确，以及防火墙设置。');
      }
      throw error;
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    // 检查是否过快地发送消息（防抖动）
    const now = Date.now();
    if (now - lastSendTimeRef.current < 1000) { // 1秒内不允许连续发送
  ...
  const handleTestSettings = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('API URL格式无效，请输入完整的URL，包括http://或https://');
      }
      
      console.log(`向 ${currentEditConfig.apiUrl} 发送测试消息: "${testPrompt}"`);
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      // 验证必填项
      if (!configToSave.name || !configToSave.apiUrl || !configToSave.apiKey || !configToSave.model) {
        setError('请填写所有必填字段');
        return;
      }
      ...
      if (window.terminalAPI && window.terminalAPI.saveApiConfig) {
        ...
        if (configToSave.id) {
          ...
          const currentSuccess = await window.terminalAPI.setCurrentApiConfig(configToSave.id);
        ...
        try {
          ...
          const settings = await window.terminalAPI.loadAISettings();
          if (settings) {
            console.log('已加载最新设置:', JSON.stringify({
            ...
            // 更新当前编辑的配置
            const savedConfig = settings.configs.find(c => c.id === configToSave.id);
            if (savedConfig) {
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                {apiSettings.configs && apiSettings.configs.length > 0 && (
                  <Box sx={{ px: 2, pt: 1, pb: 0 }}>
                    <FormControl variant="outlined" size="small" fullWidth>
                      <InputLabel id="current-api-config-label" sx={{ fontSize: '0.85rem' }}>当前API</InputLabel>
                      <Select
                ...
                <List sx={{ 
                  ...
                  {getFormattedMessages().map((message, index) => {
                    ...
                    return (
                      <MessageItem key={messageKey} isuser={message.isUser.toString()}>
                        <Box sx={{ 
                          display: 'flex', 
                          alignItems: 'flex-end', 
                          gap: 1,
                          ...
                          <MessageContent 
                            ...
                            sx={{ 
                              bgcolor: message.isError ? 'error.light' : undefined,
                              color: message.isError ? 'error.contrastText' : undefined,
                              flexGrow: message.isUser ? 0 : 1,
                              maxWidth: message.isUser ? '85%' : 'calc(100% - 40px)'
                            }}
                          >
              ...
              <Box sx={{ p: 1, display: 'flex', height: '100%' }}>
                          ...
                          >
                            <ListItemText 
                              ...
                              primaryTypographyProps={{ 
                                fontWeight: config.id === apiSettings.current?.id ? 'bold' : 'normal' 
                              }}
                            />
                          </ListItemButton>
                        </ListItem>
                ...
                <Box sx={{ width: '55%', overflow: 'auto' }}>
                  ...
                  {testResult && (
                    ...
                    >
                      <Typography variant="body2" fontWeight="medium" color={testResult.success ? 'success.dark' : 'error.dark'}>
                  ...
                  <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
                    <Button 
                      ...
                      disabled={testing || !currentEditConfig.apiUrl || !currentEditConfig.apiKey || !currentEditConfig.model}
