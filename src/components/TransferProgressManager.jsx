...
const TransferProgressItem = ({ transfer, onCancel }) => {
  ...
  return (
    ...
    >
      {/* 头部信息 - 改为更紧凑的单行布局 */}
      <Box sx={{ display: 'flex', alignItems: 'flex-start', width: '100%', mb: 0.5, gap: 0.5 }}>
        ...
        <Box sx={{ flexGrow: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          {/* 文件数量和当前文件信息 - 更紧凑 */}
          {(totalFiles > 1 || ((type === 'upload-folder' || type === 'download-folder') && currentFile)) && (
            <Typography 
              variant="caption" 
            ...
            >
              ...
              {(type === 'upload-folder' || type === 'download-folder') && currentFile && (
...
const TransferProgressManager = ({ 
  ...
  const activeTransfers = transferList.filter(t => 
    t.progress < 100 && !t.isCancelled && !t.error
  );
  ...
  return (
    ...
    >
          ...
          <Box sx={{ mt: 0.5, display: 'flex', gap: 0.5 }}>
            {activeTransfers.length > 0 && (
              <Button
                ...
                onClick={() => {
                  activeTransfers.forEach(t => handleCancelTransfer(t.transferId));
                }}
              >
                取消全部
              </Button>
      ...
      <Collapse in={isExpanded}>
        <CardContent sx={{ pt: 0, px: 1, pb: 1, maxHeight: '45vh', overflow: 'auto' }}>
          <List sx={{ p: 0 }}>
