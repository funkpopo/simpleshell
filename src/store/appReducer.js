...
export const actions = {
  ...
  updateTab: (index, tab) => ({ type: ActionTypes.UPDATE_TAB, payload: { index, tab } }),
  setCurrentTab: (index) => ({ type: ActionTypes.SET_CURRENT_TAB, payload: index }),
  setMergedTabs: (mergedTabs) => ({ type: ActionTypes.SET_MERGED_TABS, payload: mergedTabs }),
  setActiveSplitTab: (tabId) => ({ type: ActionTypes.SET_ACTIVE_SPLIT_TAB, payload: tabId }),
  
  // Drag Actions
  setDraggedTab: (index) => ({ type: ActionTypes.SET_DRAGGED_TAB, payload: index }),
  setDragOverTab: (index) => ({ type: ActionTypes.SET_DRAG_OVER_TAB, payload: index }),
  setDragOperation: (operation) => ({ type: ActionTypes.SET_DRAG_OPERATION, payload: operation }),
  setDragInsertPosition: (position) => ({ type: ActionTypes.SET_DRAG_INSERT_POSITION, payload: position }),
  resetDragState: () => ({ type: ActionTypes.RESET_DRAG_STATE }),
  
  // Sidebar Actions
  setConnectionManagerOpen: (open) => ({ type: ActionTypes.SET_CONNECTION_MANAGER_OPEN, payload: open }),
  setResourceMonitorOpen: (open) => ({ type: ActionTypes.SET_RESOURCE_MONITOR_OPEN, payload: open }),
  setFileManagerOpen: (open) => ({ type: ActionTypes.SET_FILE_MANAGER_OPEN, payload: open }),
  setIpAddressQueryOpen: (open) => ({ type: ActionTypes.SET_IP_ADDRESS_QUERY_OPEN, payload: open }),
  setRandomPasswordGeneratorOpen: (open) => ({ type: ActionTypes.SET_RANDOM_PASSWORD_GENERATOR_OPEN, payload: open }),
  setShortcutCommandsOpen: (open) => ({ type: ActionTypes.SET_SHORTCUT_COMMANDS_OPEN, payload: open }),
  setCommandHistoryOpen: (open) => ({ type: ActionTypes.SET_COMMAND_HISTORY_OPEN, payload: open }),
  setActiveSidebarMargin: (margin) => ({ type: ActionTypes.SET_ACTIVE_SIDEBAR_MARGIN, payload: margin }),
  setLastOpenedSidebar: (sidebar) => ({ type: ActionTypes.SET_LAST_OPENED_SIDEBAR, payload: sidebar }),
  
  // Dialog Actions
  setAboutDialogOpen: (open) => ({ type: ActionTypes.SET_ABOUT_DIALOG_OPEN, payload: open }),
  setSettingsDialogOpen: (open) => ({ type: ActionTypes.SET_SETTINGS_DIALOG_OPEN, payload: open }),
  setTabContextMenu: (menu) => ({ type: ActionTypes.SET_TAB_CONTEXT_MENU, payload: menu }),
  
  // Theme Actions
  setDarkMode: (darkMode) => ({ type: ActionTypes.SET_DARK_MODE, payload: darkMode }),
  setThemeLoading: (loading) => ({ type: ActionTypes.SET_THEME_LOADING, payload: loading }),
  
  // Terminal Actions
  setTerminalInstances: (instances) => ({ type: ActionTypes.SET_TERMINAL_INSTANCES, payload: instances }),
  updateTerminalInstance: (id, instance) => ({ type: ActionTypes.UPDATE_TERMINAL_INSTANCE, payload: { id, instance } }),
  
  // Connection Actions
  setConnections: (connections) => ({ type: ActionTypes.SET_CONNECTIONS, payload: connections }),
  setTopConnections: (connections) => ({ type: ActionTypes.SET_TOP_CONNECTIONS, payload: connections }),
  
  // File Manager Actions
  setFileManagerPaths: (paths) => ({ type: ActionTypes.SET_FILE_MANAGER_PATHS, payload: paths }),
  updateFileManagerPath: (tabId, path) => ({ type: ActionTypes.UPDATE_FILE_MANAGER_PATH, payload: { tabId, path } }),
  
  // Process Cache Actions
  setProcessCache: (cache) => ({ type: ActionTypes.SET_PROCESS_CACHE, payload: cache }),
  updateProcessCache: (sessionId, processId) => ({ type: ActionTypes.UPDATE_PROCESS_CACHE, payload: { sessionId, processId } }),
  
  // AI Chat Actions
  setAiChatStatus: (status) => ({ type: ActionTypes.SET_AI_CHAT_STATUS, payload: status }),
  setAiInputPreset: (preset) => ({ type: ActionTypes.SET_AI_INPUT_PRESET, payload: preset }),
