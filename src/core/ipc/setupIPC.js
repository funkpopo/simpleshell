...
const LocalTerminalHandlers = require("./handlers/localTerminalHandlers");
        ...
        return new Promise((resolve, reject) => {
          ...
          ssh.shell(
            ...
            (err, stream) => {
              ...
              // 注册SSH连接的延迟检测（复用连接）
              const latencyHandlers = ipcSetup.getLatencyHandlers();
          ...
          ssh.on("ready", () => {
            ...
            ssh.shell(
              ...
              (err, stream) => {
                ...
                // 注册SSH连接的延迟检测
                const latencyHandlers2 = ipcSetup.getLatencyHandlers();
                if (latencyHandlers2 && sshConfig.tabId) {
                  try {
                    latencyHandlers2.latencyService.registerSSHConnection(
                      sshConfig.tabId,
                      ssh,
                      sshConfig.host,
                      sshConfig.port || 22,
    ...
    if (proc && proc.process) {
      try {
        ...
        if (proc.type === "ssh2" && proc.connectionInfo) {
          ...
          // 注销延迟检测
          const latencyHandlers3 = ipcSetup.getLatencyHandlers();
          if (latencyHandlers3 && proc.config?.tabId) {
            try {
              latencyHandlers3.latencyService.unregisterConnection(
                proc.config.tabId,
              );
