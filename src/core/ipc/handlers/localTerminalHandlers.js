...
class LocalTerminalHandlers {
    ...
    try {
      ...
      for (const windowInfo of embeddedWindows) {
        if (windowInfo.isEmbedded) {
          ...
          await this.windowEmbedder.resizeEmbeddedWindow(windowInfo.tabId, containerBounds);
        }
      }
    } catch (error) {
  ...
  registerHandlers() {
    ...
    ipcMain.handle('resizeEmbeddedTerminal', async (event, tabId, bounds) => {
      try {
        const success = await this.windowEmbedder.resizeEmbeddedWindow(tabId, bounds);
