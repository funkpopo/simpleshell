...
const POSIX_SHELL_DETECTION_COMMAND = [
  'if [ -n "${SHELL:-}" ]; then printf "%s\\n" "$SHELL"; fi',
  "ps -p $$ -o comm= 2>/dev/null",
  'if command -v getent >/dev/null 2>&1; then getent passwd "${USER:-$(id -un 2>/dev/null)}" 2>/dev/null | cut -d: -f7; fi',
  'if [ -r /etc/passwd ]; then grep "^${USER:-$(id -un 2>/dev/null)}:" /etc/passwd 2>/dev/null | head -n 1 | cut -d: -f7; fi',
].join("; ");

const POWERSHELL_SHELL_DETECTION_COMMAND = [
  "if ($PSVersionTable) {",
  '  Write-Output "pwsh"',
  "}",
].join(" ");

const normalizeShellIntegrationShell = (value) => {
...
const collectShellDetectionCandidates = (stdout = "", stderr = "") =>
  `${stdout || ""}\n${stderr || ""}`
    .split(/\r?\n/)
    .map((line) => normalizeShellIntegrationShell(line))
    .filter(Boolean);
...
class TerminalHandlers {
  ...
  getHandlers() {
    return [
      ...
      {
        channel: "terminal:detectShellIntegrationShell",
        category: "terminal",
        handler: this.detectShellIntegrationShell.bind(this),
      },
      {
  ...
  _executeSSHProbe(sshClient, command, options = {}) {
    const timeoutMs = Math.max(
      1000,
      Math.floor(Number(options.timeoutMs) || 5000),
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutHandle = setTimeout(() => {
        finalize(() => {
          ...
          reject(new Error("Shell detection probe timed out"));
        });
      }, timeoutMs);
  ...
  async detectShellIntegrationShell(_event, processId) {
    const procInfo = this.processManager?.getProcess?.(processId);
    if (!procInfo) {
      return {
        shell: null,
        supported: false,
        source: "missing-process",
      };
    }
    ...
    if (
      Object.prototype.hasOwnProperty.call(
        procInfo,
        "detectedShellIntegrationShell",
      )
    ) {
      return {
        shell: procInfo.detectedShellIntegrationShell || null,
        supported: Boolean(procInfo.detectedShellIntegrationShell),
        source: procInfo.detectedShellIntegrationSource || "cache",
      };
    }

    const sshClient = procInfo.connectionInfo?.client || procInfo.process;
    if (!sshClient || typeof sshClient.exec !== "function") {
      procInfo.detectedShellIntegrationShell = null;
      procInfo.detectedShellIntegrationSource = "unsupported-process";
      return {
        shell: null,
        supported: false,
        source: "unsupported-process",
      };
    }
    ...
    const probes = [
      {
        source: "posix-exec",
        command: POSIX_SHELL_DETECTION_COMMAND,
      },
      {
        source: "powershell-exec",
        command: POWERSHELL_SHELL_DETECTION_COMMAND,
      },
    ];

    for (const probe of probes) {
      try {
        const result = await this._executeSSHProbe(sshClient, probe.command);
        const candidates = collectShellDetectionCandidates(
          result.stdout,
          result.stderr,
        );

        if (candidates.length > 0) {
          procInfo.detectedShellIntegrationShell = candidates[0];
          procInfo.detectedShellIntegrationSource = probe.source;
        ...
        logToFile(
          `Shell detection probe failed for process ${processId} (${probe.source}): ${error.message}`,
          "DEBUG",
        );
      }
    }
    ...
    procInfo.detectedShellIntegrationShell = null;
    procInfo.detectedShellIntegrationSource = "unknown";
    return {
      shell: null,
      supported: false,
      source: "unknown",
