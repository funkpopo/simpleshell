...
const api = {
  ...
  // 本地终端相关
  createLocalTerminal: async (params: {
    cols: number;
    rows: number;
  ...
  onSshClose: (
    ...
    const listener = (
      _: unknown,
      data: { id: string; code: number; signal?: string },
    ) => callback(data);
      ...
      if (data && data.id) {
        ...
        if (typeof data.id === "string" && data.id.startsWith("term_")) {
          ...
          if (data.data && typeof data.data === "string") {
            ...
            console.error(
              `监听器[${callbackId}]收到的终端[${data.id}]数据无效:`,
              typeof data.data,
            );
          }
        } else {
          console.warn(
            `监听器[${callbackId}]收到非标准格式的终端ID[${data.id}]`,
          );
  ...
  // 窗口状态监听（焦点变化等）
  onWindowStateChange: (
    callback: (data: WindowState) => void,
  ): (() => void) => {
  ...
  // 加载设置
  loadSettings: () =>
    ipcRenderer.invoke("load-settings") as Promise<AppSettings>,
