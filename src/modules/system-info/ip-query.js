...
const KEY_API_PROVIDERS = {
  ...
  ip2location: {
    ...
    transform: (data, ip) => {
        if (data.error) {
            throw new Error(`ip2location.io API error: ${data.error.error_message}`);
        }
        return {
            ret: "ok",
            data: {
...
async function getPublicIp() {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (e) => reject(new Error(`getPublicIp request error: ${e.message}`)));
    req.end();
  });
}

function fetchIpInfo(provider, ip, logger) {
    const url = provider.key ? provider.buildUrl(ip, provider.key) : provider.buildUrl(ip);
    ...
    return new Promise((resolve, reject) => {
        ...
        const req = requestModule.request(options, (res) => {
            if (res.statusCode !== 200) {
                return reject(new Error(`API ${provider.name} HTTP Error: ${res.statusCode}`));
            }
            let data = "";
            res.on("data", (chunk) => (data += chunk));
            res.on("end", () => {
                try {
                    ...
                    reject(new Error(`API ${provider.name} parsing error: ${error.message}`));
                }
            });
        });

        req.on("error", (error) => reject(new Error(`API ${provider.name} request error: ${error.message}`)));
        req.on("timeout", () => {
            req.destroy();
  ...
  try {
    ...
    for (const key in KEY_API_PROVIDERS) {
        if (KEY_API_PROVIDERS[key].key) {
            logger(`${KEY_API_PROVIDERS[key].name} API Key已配置，启用该服务。`, "INFO");
    ...
    if (ip) {
      ...
      const promises = lookupProviders.map(provider => fetchIpInfo(provider, ip, logger));
      ...
      const chinesePromise = providers.chinese ? fetchIpInfo(providers.chinese, '', logger) : Promise.reject(new Error("Chinese provider not configured"));
      const geoPromise = providers.geo ? fetchIpInfo(providers.geo, '', logger) : Promise.reject(new Error("Geo provider not configured"));

      const results = await Promise.allSettled([chinesePromise, geoPromise]);
      const chineseResult = results[0].status === 'fulfilled' ? results[0].value : null;
      const geoResult = results[1].status === 'fulfilled' ? results[1].value : null;
      ...
      if (chineseResult || geoResult) {
        ...
        if ((!finalData.latitude || !finalData.longitude) && finalData.location && finalData.location.length > 0) {
            const address = finalData.location.slice(0, 3).join('');
            logger(`尝试通过地址进行地理编码: ${address}`, "INFO");
            const geoCoords = await geocodeWithAmap(address, KEY_API_PROVIDERS.amap.key, logger);
            if (geoCoords) {
                finalData.latitude = geoCoords.latitude;
                finalData.longitude = geoCoords.longitude;
                logger(`地理编码成功: ${geoCoords.latitude}, ${geoCoords.longitude}`, "INFO");
            }
        }
      ...
      const standardLookupPromise = (async () => {
        ...
        const lookupProviders = allProviders.filter(p => !p.ownIpOnly);
        const standardPromises = lookupProviders.map(provider => fetchIpInfo(provider, publicIp, logger));
    ...
    if (typeof logger === "function") {
        const errorMessages = error.errors ? error.errors.map(e => e.message).join(", ") : error.message || "Unknown error";
