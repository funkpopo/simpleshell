...
export class ImageSupportManager {
  ...
  async initialize(terminal) {
    try {
      ...
      logToFile(`内存限制: ${Math.round(support.memoryLimit / 1024 / 1024)}MB`, 'DEBUG');
  ...
  handleImageLoad(event) {
    ...
    logToFile(`图像加载成功: ${imageId} (${width}x${height}, ${Math.round(size/1024)}KB)`, 'DEBUG');
  }
  ...
  getImageStats() {
    const stats = {
      ...
      usagePercent: (this.memoryUsage / (this.options.storageLimit * 1024 * 1024)) * 100
    };

    if (this.imageAddon) {
      stats.storageUsage = this.imageAddon.storageUsage || 0;
      stats.storageLimit = this.imageAddon.storageLimit || this.options.storageLimit;
    }
