...
const HIGHLIGHT_COLORS = { // 统一定义颜色，方便复用和修改
...
module.exports = [
  ...
  // 添加时间戳匹配
  {
    ...
    pattern: '\\b(20|21|22|23|[0-1]\\d):[0-5]\\d:[0-5]\\d\\b|\\b[1-9]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])\\s+(20|21|22|23|[0-1]\\d):[0-5]\\d:[0-5]\\d\\b|\\b\\d{4}-\\d{2}-\\d{2}\\b',
    flags: 'g',
  ...
  // 添加告警信息匹配
  {
    ...
    name: 'Alert Patterns',
    enabled: true,
    pattern: '(?:\\b(?:ALERT|ATTENTION|CAUTION|DANGER|EMERGENCY|ERROR|FAILURE|FAILED|FATAL|INVALID|WARNING|WARN)\\b)',
    flags: 'gi', // 不区分大小写
  ...
  // 添加超链接高亮规则
  {
    ...
    enabled: true,
    pattern: '(?:https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]|www\\.[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]',
    flags: 'gi', // 不区分大小写
  ...
  // 添加状态码高亮规则
  {
    ...
    enabled: true,
    // 匹配HTTP状态码和常见退出码
    pattern: '\\b(?:status(?:\\s+code)?[:=]?\\s*|HTTP[/\\\\]\\d\\.\\d\\s+)(\\d{3})\\b|\\bexit(?:ed)?\\s+code:?\\s*(\\d+)\\b',
    flags: 'gi',
