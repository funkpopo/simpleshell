...
function setupIPC(mainWindow) {
  ...
  // 检测连接延迟
  ipcMain.handle("connection:checkLatency", async (event, options) => {
    try {
      const { type, tabId, host, port } = options;
      let latency = 0;
      let targetHost = host;
      ...
      if (type === 'ssh' && tabId) {
        // SSH连接延迟检测
        const processInfo = childProcesses.get(tabId);
        if (processInfo && processInfo.type === 'ssh2' && processInfo.stream) {
          const startTime = Date.now();
          
          return new Promise((resolve, reject) => {
            // 发送一个简单的echo命令来测试延迟
            const testCommand = 'echo "ping_test_' + Date.now() + '"\n';
            
            let responded = false;
            const timeoutId = setTimeout(() => {
              if (!responded) {
                responded = true;
                resolve({ success: true, data: { latency: 999, host: targetHost } });
              }
            }, 5000);

            const onData = (data) => {
              if (!responded && data.toString().includes('ping_test_')) {
                responded = true;
                clearTimeout(timeoutId);
                const endTime = Date.now();
                latency = endTime - startTime;
                processInfo.stream.removeListener('data', onData);
                resolve({ success: true, data: { latency, host: targetHost } });
              }
            };

            processInfo.stream.on('data', onData);
            processInfo.stream.write(testCommand);
          });
        } else {
        ...
        const startTime = Date.now();
          ...
          return new Promise((resolve) => {
            dns.resolve('google.com', (err) => {
              const endTime = Date.now();
              const latency = err ? 999 : (endTime - startTime);
              resolve({ 
                success: true, 
                data: { 
                  latency,
      ...
      return { success: false, error: '不支持的延迟检测类型' };
    } catch (error) {
      logToFile(`延迟检测失败: ${error.message}`, 'ERROR');
      return { success: false, error: error.message };
    }
  });
